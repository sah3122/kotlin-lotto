...
class LottoMachineTest : StringSpec({
    "로또 기계를 생성할수 있다" {
        shouldNotThrow<Throwable> { LottoMachine(Purchase.create(2_000, 1)) }
    }

    "자동 로또를 구입할수 있다" {
        val purchase = Purchase.create(2_000, 0)
    ...
    "수동 로또를 구입할수 있다" {
        val purchase = Purchase.create(2_000, 1)
    ...
    "수동 로또 구입수와 입력한 수동 로또 번호수가 일치 하지 않으면 Exception을 던진다" {
        val purchase = Purchase.create(2_000, 2)
    ...
    companion object {
        private val lottoMachine = LottoMachine(Purchase.create(2_000, 1))
    }
}
